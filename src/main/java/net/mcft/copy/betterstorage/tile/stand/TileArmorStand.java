...
public class TileArmorStand extends TileContainerBetterStorage {
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumFacing side, float hitX, float hitY, float hitZ){
		...
		if (((EnumFacing) world.getBlockState(pos).getValue(FACING)).getIndex() > 0) { pos = pos.offset(EnumFacing.DOWN); hitY += 1; }
		return super.onBlockActivated(world, pos, state, playerIn, side, hitX, hitY, hitZ);
	}
	@Override
	public ItemStack getPickBlock(MovingObjectPosition target, World world, BlockPos pos) {
		if (((EnumFacing) world.getBlockState(pos).getValue(FACING)).getIndex() > 0) { pos = pos.offset(EnumFacing.DOWN); }
		return super.getPickBlock(target, world, pos);
	}
	
	@Override
	public void breakBlock(World worldIn, BlockPos pos, IBlockState state){
		if (((EnumFacing) state.getValue(FACING)).getIndex() > 0) return;
		super.breakBlock(worldIn, pos, state);
	}
	
	@Override
	public void onNeighborBlockChange(World world, BlockPos pos, IBlockState state, Block neighborBlock) {
		int metadata = ((EnumFacing) state.getValue(FACING)).getIndex();
		int yModifier = ((metadata == 0) ? 1 : -1);
		int targetMeta = ((metadata == 0) ? 1 : 0);
		if ((world.getBlockState(pos.offset(EnumFacing.UP, yModifier)).getBlock() == this) &&
		    (((EnumFacing) world.getBlockState(pos.offset(EnumFacing.UP, yModifier)).getValue(FACING)).getIndex()) == targetMeta) return;
