...
public class ItemBackpack extends ItemArmorBetterStorage implements ISpecialArmor, IDyeableItem {
	...
	public static final ArmorMaterial material = EnumHelper.addArmorMaterial(
			"backpack", Constants.modId + "backpack", -1, new int[]{ 0, 2, 0, 0 }, 15);
	...
	*/
	...
	protected boolean takesDamage(ItemStack stack, DamageSource source) {
		...
		return (!enchantmentProtection(stack, Enchantment.protection, 0.3, 0.35, 0.4, 0.45) && //protection
		        !(source.isProjectile() && enchantmentProtection(stack, Enchantment.projectileProtection, 0.4, 0.5, 0.6, 0.7)) && //projectileProtection
	...
	public static boolean openBackpack(EntityPlayer player, EntityLivingBase carrier) {
		...
		player.openGui(BetterStorage.instance, Constants.guiBackpackID, player.worldObj, backpackType.getBackpackColumns(), backpackType.getBackpackRows(), 0);
	...
	public static boolean placeBackpack(EntityLivingBase carrier, EntityPlayer player, ItemStack backpack, 
		...
		Block blockBelow = world.getBlockState(pos.offset(EnumFacing.DOWN)).getBlock();
		if ((deathDrop ? blockBelow.isReplaceable(world, pos.offset(EnumFacing.DOWN))
		               : !world.isSideSolid(pos.offset(EnumFacing.DOWN), EnumFacing.UP))) return false;
		...
		if ((player != null) && (!world.isBlockModifiable(player, pos) ||
		                         !player.canPlayerEdit(pos, side, backpack))) return false;
		...
		if (!world.setBlockState(pos, blockBackpack.getDefaultState().withProperty(TileBackpack.FACING, orientation), SetBlockFlag.DEFAULT))
			return false;
