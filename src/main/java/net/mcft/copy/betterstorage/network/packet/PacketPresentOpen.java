		...
		if (present != null)
			for (int side = 0; side < 6; side++)
				for (int i = 0; i < (destroy ? 2 : 20); i++)
					if (destroy)
						effRender.addBlockDestroyEffects(pos, player.worldObj.getBlockState(pos)); //TODO (1.8): Not sure if addBlockDestroyEffects
					else effRender.addBlockHitEffects(pos, EnumFacing.getFront(side)); //addBlockHitEffects
	}
